...
import { upsertFcmToken } from "@/lib/notifications/inbox";
...
export async function POST(request: Request) {
  ...
  const r = resolveRecipient(cookieStore, searchParams.get("for"));
  if ("error" in r) {
  ...
  try {
    await upsertFcmToken(r.audience, r.recipientKey, parsed.data.token.trim());
